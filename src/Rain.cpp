...
int Rain::RenderParticles(VECTOR_CLASS* rminu,VECTOR_CLASS* rpluu,float processParts)
{
...
void Rain::ResetPart(int ID,int forced)
{
	if(!forced)
	{
		float globalFade=1-(float)(GetTickCount()-lifeStart)/(float)life;
	...
	particles[ID].pos=VECTOR_CLASS(rand()%1400-600,600+rand()%100,0);
	float speed=(float)(rand()%1000+500)/50;
	particles[ID].vel=VECTOR_CLASS(speed,-speed,0);

	if(forced)	particles[ID].life = ID;
	else		particles[ID].life=(abs((rand()%500-250))+life);
}

void Rain::GiveData(void* data)
{
	if(data)
	{
		RainData* sData=(RainData*)data;
		size=sData->size;
		life=sData->life;
		radius=sData->radius;
		done = 0;
