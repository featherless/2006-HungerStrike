...
void LoseModule::GiveData(void* data)
{}

void LoseModule::Initialize()
{
	sfx.PlaySFX(GAMELOSE);
	startTick=GetTickCount();
	stage=FADEIN;
}

Module* LoseModule::ExecuteModule()
{
	if(stage==FADEIN)
	{
	...
	else if(stage==FADEOUT)
	{
		...
		sfx.SetVolume(GAMELOSE,(1-diff)*255);
	...
	SetTexture1(game.Textures[LOSE]);
	glLoadIdentity();
	glBegin(GL_QUADS);
